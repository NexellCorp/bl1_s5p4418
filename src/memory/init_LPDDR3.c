			      ...
			      U16 value)
{
	...
	WriteIO32(&pReg_Drex->DIRECTCMD,
		  (U32)(cmd << 24 | chipnum << 20 | mrx << 16 | value));
#endif
...
void enterSelfRefresh(void)
{
...
#if 0	// PASR Bank Command (Optional)
	unsigned short MR16 = 0xFF << 0;
	unsigned short MR17 = 0xFF << 0;

	// Send MR16 PASR_Bank command.
	SendDirectCommand(SDRAM_CMD_MRS, 0, 16, MR16);
#if (CFG_NSIH_EN == 0)
#if (_DDR_CS_NUM > 1)
	SendDirectCommand(SDRAM_CMD_MRS, 1, 16, MR16);
#endif
#else
	if(pSBI->DII.ChipNum > 1)
	...
	SendDirectCommand(SDRAM_CMD_MRS, 0, 17, MR17);
#if (CFG_NSIH_EN == 0)
#if (_DDR_CS_NUM > 1)
	SendDirectCommand(SDRAM_CMD_MRS, 1, 17, MR17);
#endif
#else
	if(pSBI->DII.ChipNum > 1)
		SendDirectCommand(SDRAM_CMD_MRS, 1, 17, MR17);
	...
	ClearIO32(&pReg_Drex->CONCONTROL,
	    (0x1<< 5)); // afre_en[5]. Auto Refresh Counter. Disable:0, Enable:1
...
void exitSelfRefresh(void)
{
	...
	// Step 10    ACK, ACKB on
	ClearIO32(&pReg_Drex->MEMCONTROL,
		  (0x1 << 0)); // clk_stop_en[0] : Dynamic
			       // Clock Control   :: 1'b0  -
			       // Always running
	DMC_Delay(10);

	// Step 52 Auto refresh counter enable
	SetIO32(&pReg_Drex->CONCONTROL,
		(0x1<< 5)); // afre_en[5]. Auto Refresh Counter. Disable:0, Enable:1
	DMC_Delay(10);
...
void DDR_Write_Leveling(void)
{
	...
	MR1.MR1.QOff = 0;
	MR1.MR1.RTT_Nom2 =
	    0; // RTT_Nom - 001: RZQ/4, 010: RZQ/2, 011: RZQ/6, 100:
	       // RZQ/12, 101: RZQ/8
	MR1.MR1.RTT_Nom1 = 1;
	MR1.MR1.RTT_Nom0 = 0;
	MR1.MR1.WL = 1;
	...
	temp =
	    ((0x8 << 24) | (0x8 << 17) | (0x8 << 8) |
	     (0x8
	      << 0)); // PHY_CON30[30:24] = ctrl_wrlvl_code3, PHY_CON30[23:17]
		      // = ctrl_wrlvl_code2, PHY_CON30[14:8] =
		      // ctrl_wrlvl_code1, PHY_CON30[6:0] = ctrl_wrlvl_code0
	...
	temp = ReadIO32(
	    &pReg_DDRPHY
		 ->PHY_CON[30 + 1]); // PHY_CON30[30:24] = ctrl_wrlvl_code3,
...
CBOOL DDR_CA_Calibration(void)
{
	...
	while (1) {
		...
		SendDirectCommand(SDRAM_CMD_MRS, 0, 41,
				  0xA4); //- CH0 : Send MR41 to start
// CA calibration for LPDDR3 :
// MA=0x29 OP=0xA4, 0x50690
#if (CFG_NSIH_EN == 0)
#if (_DDR_CS_NUM > 1)
		SendDirectCommand(SDRAM_CMD_MRS, 1, 41,
				  0xA4); //- CH1 : Send MR41 to start
// CA calibration for LPDDR3 :
// MA=0x29 OP=0xA4, 0x50690
#else
		if (pSBI->DII.ChipNum > 1)
			SendDirectCommand(SDRAM_CMD_MRS, 1, 41,
					  0xA4); //- CH1 : Send MR41 to
						 // start CA calibration for
						 // LPDDR3 : MA=0x29 OP=0xA4,
						 // 0x50690
#endif
#endif
		...
		WriteIO32(&pReg_Drex->CACAL_CONFIG[1],
			  0x00000001); // cacal_csn(dfi_csn_p0)[0]=1 : generate
				       // one pulse
		// CSn(Low and High), cacal_csn field need not to
		// return to "0" and whenever this field is written
		// in "1", one pulse is genrerated.
		//        DMC_Delay(0x80);
	...
	vwmc = (vwml + vwmr) >> 1;
	code = (int)(vwmc -
		     lock_div4); //- (g_DDRLock >> 2) means "T/4", lock value
...
ca_error_ret:
	...
	SendDirectCommand(SDRAM_CMD_MRS, 0, 42,
			  0xA8); // CH0 : Send MR42 to exit from
				 // CA calibration mode for
				 // LPDDR3, MA=0x2A OP=0xA8,
				 // 0x50AA0
#if (CFG_NSIH_EN == 0)
#if (_DDR_CS_NUM > 1)
	SendDirectCommand(SDRAM_CMD_MRS, 1, 42,
			  0xA8); // CH1 : Send MR42 to exit from
				 // CA calibration mode for
				 // LPDDR3, MA=0x2A OP=0xA8,
				 // 0x50AA0
#else
	if (pSBI->DII.ChipNum > 1)
		SendDirectCommand(SDRAM_CMD_MRS, 1, 42,
				  0xA8); // CH1 : Send MR42 to exit
					 // from CA calibration mode
					 // for LPDDR3, MA=0x2A
					 // OP=0xA8, 0x50AA0
#endif
#endif
...
CBOOL DDR_Gate_Leveling(U32 isResume)
{
	...
	SetIO32(&pReg_DDRPHY->PHY_CON[14],
		(0xF << 0)); // ctrl_pulld_dqs[3:0] = 0
	SetIO32(&pReg_DDRPHY->PHY_CON[0], (0x1 << 13)); // byte_rdlvl_en[13]=1

	if (isResume == 0) {
		SendDirectCommand(SDRAM_CMD_PALL, 0, (SDRAM_MODE_REG)CNULL,
				  CNULL);
#if (CFG_NSIH_EN == 0)
#if (_DDR_CS_NUM > 1)
		SendDirectCommand(SDRAM_CMD_PALL, 1, (SDRAM_MODE_REG)CNULL,
...
#else
		if (pSBI->DII.ChipNum > 1)
			SendDirectCommand(SDRAM_CMD_PALL, 1,
	...
	temp = ReadIO32(&pReg_DDRPHY->PHY_CON[1]) &
	       ~(0xF << 20); // ctrl_gateduradj[23:20] = DDR3: 0x0, LPDDR3: 0xB,
			     // LPDDR2: 0x9
	temp |= (0xB << 20);
	WriteIO32(&pReg_DDRPHY->PHY_CON[1], temp);
#endif
	...
	/* Update reset DLL */
	WriteIO32(&pReg_Drex->RDLVL_CONFIG,
		  (0x1 << 0)); // ctrl_rdlvl_gate_en[0] = 1

	if (isResume) {
		while ((ReadIO32(&pReg_Drex->PHYSTATUS) & (0x1 << 14)) == 0)
			; // rdlvl_complete[14] = 1
		WriteIO32(&pReg_Drex->RDLVL_CONFIG,
			  0); // ctrl_rdlvl_gate_en[0] = 0
	} else {
		while ((ReadIO32(&pReg_Drex->PHYSTATUS) & (0x1 << 14)) ==
		       0) // rdlvl_complete[14] : Wating until READ leveling is
			  // complete
		{
			if (cal_count > 100) // Failure Case
			{
				WriteIO32(&pReg_Drex->RDLVL_CONFIG,
					  0x0); // ctrl_rdlvl_data_en[0]=0 :
						// Stopping it after
						// completion of READ
						// leveling.
		...
		WriteIO32(&pReg_Drex->RDLVL_CONFIG,
			  0x0); // ctrl_rdlvl_data_en[0]=0 :
				// Stopping it after completion of
				// READ leveling.

		//- Checking the calibration failure status
		//- After setting PHY_CON5(0x14) to "0xC", check whether
		//PHY_CON21 is zero
		// or nor. If PHY_CON21(0x58) is zero, calibration is normally
		// complete.

		WriteIO32(&pReg_DDRPHY->PHY_CON[5],
			  VWM_FAIL_STATUS); // readmodecon[7:0] = 0xC
		cal_count = 0;
		do {
			if (cal_count > 100) {
				MEMMSG("\r\n\nRD VWM_FAIL_STATUS Checking : "
				       "fail...!!!\r\n");
...
#if (DDR_GATE_LVL_COMPENSATION_EN == 1)
	{
		...
		//    temp = ( ((U8)offsetc[3] << 24) | ((U8)offsetc[2] << 16) |
		//    ((U8)offsetc[1] << 8) | (U8)offsetc[0] );
	...
	temp = (((U8)gate_cycle[3] << 15) | ((U8)gate_cycle[2] << 10) |
		((U8)gate_cycle[1] << 5) | (U8)gate_cycle[0]);
	WriteIO32(&pReg_DDRPHY->PHY_CON[3], temp); // ctrl_shiftc
#endif // #if (DDR_RESET_GATE_LVL == 1)

#if (MEM_CALIBRAION_INFO == 1)
	DDR_Gate_Leveling_Information();
#endif
...
gate_err_ret:
	...
	SetIO32(&pReg_Drex->PHYCONTROL[0],
		(0x1 << 3)); // Force DLL Resyncronization
	ClearIO32(&pReg_Drex->PHYCONTROL[0],
		  (0x1 << 3)); // Force DLL Resyncronization
...
CBOOL DDR_Read_DQ_Calibration(U32 isResume)
{
	...
	WriteIO32(
	    &pReg_Drex->RDLVL_CONFIG,
	    (0x1 << 1)); // ctrl_rdlvl_data_en[1]=1 : Starting READ leveling

	if (isResume) {
		while ((ReadIO32(&pReg_Drex->PHYSTATUS) & (0x1 << 14)) == 0)
			; // rdlvl_complete[14] : Wating until READ leveling is
			  // complete
		WriteIO32(&pReg_Drex->RDLVL_CONFIG,
			  0x0); // ctrl_rdlvl_data_en[1]=0 :
				// Stopping it after completion of
				// READ leveling.
	} else {
		while ((ReadIO32(&pReg_Drex->PHYSTATUS) & (0x1 << 14)) ==
	...
	//*** Forcing manually offsetr to stop the calibration mode after
	//completion
	// of READ DQ Cal
...
#if (DDR_READ_DQ_COMPENSATION_EN == 1)
	{
		...
		//    temp = ( ((U8)offsetr[3] << 24) | ((U8)offsetr[2] << 16) |
		//    ((U8)offsetr[1] << 8) | (U8)offsetr[0] );
	...
	//*** Resync
	//*** Update READ SDLL Code (ctrl_offsetr) : Make "ctrl_resync" HIGH and
	//LOW
	SetIO32(&pReg_DDRPHY->PHY_CON[10],
		(0x1 << 24)); // ctrl_resync[24]=0x1 (HIGH)
	ClearIO32(&pReg_DDRPHY->PHY_CON[10],
...
rd_err_ret:
	...
	//*** Set PHY0.CON2.rdlvl_en : setting MUX as "0" to force manually the
	//result
	// value of READ leveling.
...
CBOOL DDR_Write_DQ_Calibration(U32 isResume)
{
	...
	ClearIO32(&pReg_DDRPHY->PHY_CON[0],
		  (0x1 << 5)); // ctrl_read_disable[5]=0.
			       // Read ODT disable signal.
			       // Variable. Set to '1', when
			       // you need Read Leveling
			       // test.
...
#if (DDR_WRITE_DQ_COMPENSATION_EN == 1)
	{
		...
		//    temp = ( ((U8)offsetw[3] << 24) | ((U8)offsetw[2] << 16) |
		//    ((U8)offsetw[1] << 8) | (U8)offsetw[0] );
	...
	//*** Resync
	//*** Update WRITE SDLL Code (ctrl_offsetr) : Make "ctrl_resync" HIGH
	//and LOW
	SetIO32(&pReg_DDRPHY->PHY_CON[10],
		(0x1 << 24)); // ctrl_resync[24]=0x1 (HIGH)
	ClearIO32(&pReg_DDRPHY->PHY_CON[10],
...
wr_err_ret:
	...
	ClearIO32(&pReg_DDRPHY->PHY_CON[2],
		  (0x3 << 26)); // wr_deskew_en[27] = 0, wr_deskew_con[26] = 0
...
//    DMC_Delay(0x10000);                                        // wait 300ms
	...
	if (isResume) {
		...
		g_CAvwmc =
		    ReadIO32(&pReg_RTC->RTCSCRATCH); // read - ctrl_offsetd

		WriteIO32(&pReg_Alive->ALIVEPWRGATEREG,
			  0); // close alive power gate
		WriteIO32(&pReg_Alive->ALIVEPWRGATEREG,
			  0); // close alive power gate
	}
...
// pSBI->LvlTr_Mode    = 0;
	...
	//    010:
	//    RZQ/2, 011: RZQ/6, 100: RZQ/12, 101: RZQ/8
...
#if defined(MEM_TYPE_LPDDR23)
#if (DDR_CA_SWAP_MODE == 1)
	WriteIO32(&pReg_DDRPHY->PHY_CON[22 + 1],
		  0x00000041); // lpddr2_addr[19:0] = 0x041
#else
	WriteIO32(&pReg_DDRPHY->PHY_CON[22 + 1],
		  0x00000208); // lpddr2_addr[19:0] = 0x208
#endif

	WriteIO32(
	    &pReg_DDRPHY->PHY_CON[1],
	    (0x0 << 28) |     // [31:28] ctrl_gateadj
		(0x9 << 24) | // [27:24] ctrl_readadj
		(0xB
		 << 20) | // [23:20] ctrl_gateduradj  :: DDR3: 0x0, LPDDR3: 0xB,
			  // LPDDR2: 0x9
		(0x1 << 16) |   // [19:16] rdlvl_pass_adj
		(0x0001 << 0)); // [15: 0] rdlvl_rddata_adj :: LPDDR3 : 0x0001
				// or 0x00FF
//        (0x00FF <<   0) );          // [15: 0] rdlvl_rddata_adj :: LPDDR3 :
//        0x0001 or 0x00FF
#endif
	...
	WriteIO32(&pReg_DDRPHY->PHY_CON[2],
		  (0x0 << 28) |     // [31:28] ctrl_readduradj
		      ...
		      (0x2 << 10) | // [11:10] rdlvl_start_adj - Right shift,
				    // valid value: 1
				    // or 2
		      (0x1 << 8) |  // [ 9: 8] rdlvl_start_adj - Left shift,
				    // valid value: 0 ~ 2
	...
	temp =
	    ((0x0 << 29) |  // [31:29] Reserved - SBZ.
...
#endif
#endif
	     (0x0 << 16) | // [   16] ctrl_wrlvl_en[16]. Write Leveling Enable.
			   // 0:Disable, 1:Enable
	     //        (0x0    <<  15) |           // [   15] Reserved SBZ.
	     (0x0
	      << 14) | // [   14] p0_cmd_en. 0:Issue Phase1 Read command during
		       // Read Leveling. 1:Issue Phase0
	     ...
	     //        (0x1    <<   8) |           // [    8] ctrl_shgate.
	     //        0:Gate
	     //        signal length=burst length/2+N, 1:Gate signal
	     //        length=burst
	     //        length/2-1
	     (0x0
	      << 7) | // [    7] ctrl_ckdis. 0:Clock output Enable, 1:Disable
//        (0x1    <<   6) |           // [    6] ctrl_atgate.
#if defined(MEM_TYPE_DDR3)
	     (0x1 << 5) | // [    5] ctrl_read_disable. Read ODT disable signal.
// Variable. Set to '1', when you need Read Leveling test.
#endif
...
#if defined(MEM_TYPE_LPDDR23)
	temp = ((0x105E << 16) |
		0x000E); // cmd_active= DDR3:0x105E, LPDDDR2 or LPDDDR3:0x000E
...
/* ZQ Calibration */
...
#else
		(pSBI->PHY_DSInfo.ZQ_DDS << 24) | // [26:24] zq_mode_dds, Driver
						  // strength selection. 100 :
		// 48ohm, 101 : 40ohm, 110 : 34ohm, 111 : 30ohm
		(pSBI->PHY_DSInfo.ZQ_ODT
		 << 21) | // [23:21] ODT resistor value. 001 :
			  // 120ohm, 010 : 60ohm, 011 : 40ohm,
			  // 100 : 30ohm
#endif
		(0x0
...
#endif
		(0x1
		 << 18) | // [   18] zq_clk_div_en. Clock Dividing Enable : 0,
			  // Disable : 1
		(0x0 << 15) | // [17:15] zq_force-impn
		(0x0 << 12) | // [14:12] zq_force-impp
		(0x30 << 4) | // [11: 4] zq_udt_dly
		(0x1
		 << 2) | // [ 3: 2] zq_manual_mode. 0:Force Calibration, 1:Long
			 // cali, 2:Short cali
		(0x0
		 << 1) | // [    1] zq_manual_str. Manual Calibration Stop : 0,
			 // Start : 1
		(0x0 << 0)); // [    0] zq_auto_en. Auto Calibration enable

	SetIO32(&pReg_DDRPHY->PHY_CON[16],
		(0x1 << 1)); // zq_manual_str[1]. Manual Calibration Start=1
	while ((ReadIO32(&pReg_DDRPHY->PHY_CON[17 + 1]) & 0x1) == 0)
	...
	//    ClearIO32( &pReg_DDRPHY->PHY_CON[16],   (0x1    <<  18) ); //
	//    zq_clk_div_en[18]. Clock Dividing Enable : 1, Disable : 0

	// Step 3. Set the PHY for dqs pull down mode
	WriteIO32(
	    &pReg_DDRPHY->PHY_CON[14],
	    (0x0 << 8) |     // ctrl_pulld_dq[11:8]
		(0xF << 0)); // ctrl_pulld_dqs[7:0].  No Gate leveling : 0xF,
			     // Use Gate leveling : 0x0(X)
	// Step 4. ODT
	WriteIO32(
	    &pReg_Drex->PHYCONTROL[0],
#if (CFG_ODT_ENB == 1)
	    (0x1 << 31) | // [   31] mem_term_en. Termination Enable for memory.
			  // Disable : 0, Enable : 1
		(0x1 << 30) | // [   30] phy_term_en. Termination Enable for
			      // PHY. Disable : 0, Enable : 1
		(0x0 << 0) |  // [    0] mem_term_chips. Memory Termination
		...
		//        (0x1    <<   3) |           // [    3] fp_resync.
		//        Force DLL Resyncronization : 1. Test : 0x0
	...
	WriteIO32(
		...
		(0x1 << 8) |  // [    8] empty
		//        (0x1    <<   5) |           // [    5] aref_en - Auto
		//        Refresh Counter. Disable:0, Enable:1
		(0x0 << 3) | // [    3] io_pd_con - I/O Powerdown Control in Low
			     // Power Mode(through LPI)
		(0x0 << 1)); // [ 2: 1] clk_ratio. Clock ratio of Bus clock to
			     // Memory clock. 0x0 = 1:1, 0x1~0x3 = Reserved
#else
	...
	temp = (U32)(
	    ...
	    (0x3 << 12) |   // [14:12] rd_fetch
	    (0x1 << 8) |    // [    8] empty
	    (0x1 << 5) | // [    5] aref_en - Auto Refresh Counter. Disable:0,
			 // Enable:1
	    (0x0
	     << 3) | // [    3] io_pd_con - I/O Powerdown Control in Low Power
		     // Mode(through LPI)
	    (0x0
	     << 1)); // [ 2: 1] clk_ratio. Clock ratio of Bus clock to Memory
		     // clock. 0x0 = 1:1, 0x1~0x3 = Reserved
...
#if 1 // Why Don't Use ? (by.deoks)
	// Step 5. dfi_init_start : High
	SetIO32(&pReg_Drex->CONCONTROL,
		(0x1 << 28)); // DFI PHY initialization start
	while ((ReadIO32(&pReg_Drex->PHYSTATUS) & (0x1 << 3)) == 0)
		; // wait for DFI PHY initialization complete
	ClearIO32(&pReg_Drex->CONCONTROL,
		  (0x1 << 28)); // DFI PHY initialization clear
#endif
	...
	// Step x : MemControl and PhyControl0.

	// Step 11. MemBaseConfig
	WriteIO32(
	    &pReg_Drex->MEMBASECONFIG[0],
	    (0x0 << 16) | // chip_base[26:16]. AXI Base Address. if 0x20 ==> AXI
			  // base addr of memory : 0x2000_0000
#if (CFG_NSIH_EN == 0)
		(0x7E0 << 0)); // 256MB:0x7F0, 512MB: 0x7E0, 1GB:0x7C0, 2GB:
			       // 0x780, 4GB:0x700
#else
		(pSBI->DII.ChipMask
		 << 0)); // chip_mask[10:0]. 1GB:0x7C0, 2GB:0x780
#endif

#if (CFG_NSIH_EN == 0)
#if (_DDR_CS_NUM > 1)
	WriteIO32(
	    &pReg_Drex->MEMBASECONFIG[1],
	    (chip_base1 << 16) | // chip_base[26:16]. AXI Base Address. if 0x40
				 // ==> AXI base addr of memory : 0x4000_0000,
				 // 16MB unit
		//            (0x040      <<  16) |               //
		//            chip_base[26:16]. AXI Base Address. if 0x40 ==>
	...
	if (pSBI->DII.ChipNum > 1) {
		WriteIO32(&pReg_Drex->MEMBASECONFIG[1],
			  (pSBI->DII.ChipBase
			   << 16) | // chip_base[26:16]. AXI Base Address. if
				    // 0x40 ==> AXI base addr of memory :
				    // 0x4000_0000, 16MB unit
			      (pSBI->DII.ChipMask
			       << 0)); // chip_mask[10:0]. 2048 - chip size
	}
#endif

	// Step 12. MemConfig
	WriteIO32(
	    &pReg_Drex->MEMCONFIG[0],
	    //        (0x0    <<  16) |           // [31:16] Reserved - SBZ
	    (0x1 << 12) | // [15:12] chip_map. Address Mapping Method (AXI to
			  // Memory). 0:Linear(Bank, Row, Column, Width),
...
#if (_DDR_CS_NUM > 1)
	WriteIO32(
	    &pReg_Drex->MEMCONFIG[1],
	    //            (0x0    <<  16) |       // [31:16] Reserved - SBZ
	    (0x1 << 12) | // [15:12] chip_map. Address Mapping Method (AXI to
			  // Memory). 0 : Linear(Bank, Row, Column, Width), 1 :
			  // Interleaved(Row, bank, column, width), other :
			  // reserved
		(chip_col << 8) | // [11: 8] chip_col. Number of Column Address
				  // Bit. others:Reserved, 2:9bit, 3:10bit,
		(chip_row << 4) | // [ 7: 4] chip_row. Number of  Row Address
	...
	if (pSBI->DII.ChipNum > 1) {
		WriteIO32(
		    &pReg_Drex->MEMCONFIG[1],
		    //            (0x0    <<  16) |       // [31:16] Reserved -
		    //            SBZ
		    (0x1 << 12) | // [15:12] chip_map. Address Mapping Method
				  // (AXI to
			// Memory). 0 : Linear(Bank, Row, Column, Width), 1 :
			// Interleaved(Row, bank, column, width), other :
			// reserved
			(pSBI->DII.ChipCol
			 << 8) | // [11: 8] chip_col. Number of Column
				 // Address Bit. others:Reserved, 2:9bit,
				 // 3:10bit,
			(pSBI->DII.ChipRow
			 << 4) |     // [ 7: 4] chip_row. Number of  Row
				     // Address Bit. others:Reserved, 0:12bit,
				     // 1:13bit, 2:14bit, 3:15bit, 4:16bit
	...
	WriteIO32(&pReg_Drex->PRECHCONFIG,
		  0xFF000000); //- precharge policy counter
...
// Step 14.  AC Timing
	...
	WriteIO32(&pReg_Drex->TIMINGROW, (tRFC << 24) | (tRRD << 20) |
					     (tRP << 16) | (tRCD << 12) |
					     (tRC << 6) | (tRAS << 0));
	WriteIO32(&pReg_Drex->TIMINGDATA, (tWTR << 28) | (tWR << 24) |
					      (tRTP << 20) | (W2W_C2C << 16) |
					      (R2R_C2C << 15) | (tDQSCK << 12) |
					      (nWL << 8) | (nRL << 0));

	WriteIO32(&pReg_Drex->TIMINGPOWER, (tFAW << 26) | (tXSR << 16) |
					       (tXP << 8) | (tCKE << 4) |
					       (tMRD << 0));
	...
	WriteIO32(
	    ...
	    (0x0 << 25) | // [26:25] mrr_byte     : Mode Register Read Byte lane
			  // location
...
#if (CFG_NSIH_EN == 0)
		((_DDR_CS_NUM - 1)
		 << 16) | // [19:16] num_chip : Number of Memory
			  // Chips                :: 4'h0  - 1chips
#else
		((pSBI->DII.ChipNum - 1)
		 << 16) | // [19:16] num_chip : Number of
			  // Memory Chips                ::
			  // 4'h0  - 1chips
#endif
		(0x2 << 12) | // [15:12] mem_width    : Width of Memory Data Bus
			      // :: 4'h2  - 32bits
#if defined(MEM_TYPE_DDR3)
		(0x6 << 8) | // [11: 8] mem_type     : Type of Memory :: 4'h6  -
			     // ddr3
#endif
#if defined(MEM_TYPE_LPDDR23)
		(0x7 << 8) | // [11: 8] mem_type     : Type of Memory :: 4'h7  -
			     // lpddr3
#endif
		(0x0 << 6) | // [ 7: 6] add_lat_pall : Additional Latency for
			     // PALL in
			     // cclk cycle :: 2'b00 - 0 cycle
		(0x0 << 5) | // [    5] dsref_en     : Dynamic Self Refresh
			     // :: 1'b0  - Disable
		(0x0 << 4) | // [    4] tp_en        : Timeout Precharge
			     // :: 1'b0  - Disable
		(0x0
		 << 2) | // [ 3: 2] dpwrdn_type  : Type of Dynamic Power Down
			 // :: 2'b00 - Active/precharge power down
	...
	WriteIO32(&pReg_DDRPHY->PHY_CON[31 + 1],
		  0x0C183060); // PHY_CON31 DeSkew Code for CA
	WriteIO32(&pReg_DDRPHY->PHY_CON[32 + 1],
		  0x60C18306); // PHY_CON32 DeSkew Code for CA
	WriteIO32(&pReg_DDRPHY->PHY_CON[33 + 1],
		  0x00000030); // PHY_CON33 DeSkew Code for CA
	...
	//    SetIO32( &pReg_Drex->CONCONTROL,        (0x1    <<   5)); //
	//    afre_en[5]. Auto Refresh Counter. Disable:0, Enable:1

	// Step 16. Confirm that after RESET# is de-asserted, 500 us have passed
	// before CKE becomes active.
	// Step 17. Confirm that clocks(CK, CK#) need to be started and
	//     stabilized for at least 10 ns or 5 tCK (which is larger) before
	//     CKE
	//     goes active.
	...
	for (i = 0; i < 20000; i++) {
		SendDirectCommand(SDRAM_CMD_MRR, 0, 0,
				  0); // 0x9 = MRR (mode register reading),
				      // MR0_Device Information
		if (ReadIO32(&pReg_Drex->MRSTATUS) &
		    (1 << 0)) // OP0 = DAI (Device Auto-Initialization Status)
		{
			break;
		}
	}
	...
	SendDirectCommand(SDRAM_CMD_MRS, 0, 10, 0xFF); // 0x0 = MRS/EMRS (mode
						       // register setting),
// MR10_Calibration, 0xFF:
...
// initialization
	...
	WriteIO32(&pReg_DDRPHY->PHY_CON[31 + 1],
		  0x81020408); // PHY_CON31 DeSkew Code for CA
	WriteIO32(&pReg_DDRPHY->PHY_CON[32 + 1],
		  0x08102040); // PHY_CON32 DeSkew Code for CA
	WriteIO32(&pReg_DDRPHY->PHY_CON[33 + 1],
		  0x00000004); // PHY_CON33 DeSkew Code for CA
...
// Set PLL3.
	...
	do {
		SetIO32(&pReg_DDRPHY->PHY_CON[12],
			(0x1 << 5)); // ctrl_dll_on[5]=1

		do {
			temp = ReadIO32(
			    &pReg_DDRPHY->PHY_CON[13]); // read lock value
		} while ((temp & 0x7) != 0x7);

		ClearIO32(&pReg_DDRPHY->PHY_CON[12],
	...
	SetIO32(&pReg_DDRPHY->PHY_CON[0],
		(0x1 << 5)); // ctrl_read_disable[5]= 1.
			     // Read ODT disable signal.
			     // Variable. Set to '1', when
			     // you need Read Leveling test.
	...
	WriteIO32(&pReg_Drex->RDLVL_CONFIG,
		  0x00000001); // ctrl_rdlvl_data_en[1]=1, Gate Traning : Enable
	while ((ReadIO32(&pReg_Drex->PHYSTATUS) & 0x4000) != 0x4000)
		; // Rdlvl_complete_ch0[14]=1

	//    WriteIO32( &pReg_Drex->RDLVL_CONFIG,    0x00000000 );
	//    //- ctrl_rdlvl_data_en[1]=0, Gate Traning : Disable
	WriteIO32(&pReg_Drex->RDLVL_CONFIG, 0x00000001); // LINARO

#if defined(MEM_TYPE_DDR3)
	WriteIO32(
	    &pReg_DDRPHY->PHY_CON[14],
	    0x00000000); // ctrl_pulld_dq[11:8]=0x0, ctrl_pulld_dqs[3:0]=0x0
#endif
...
#if defined(MEM_TYPE_LPDDR23)
	    (0x3
	     << 11) | // [12:11] ctrl_ddr_mode. 0:DDR2&LPDDR1, 1:DDR3, 2:LPDDR2,
		      // 3:LPDDR3
#endif
	    //        (0x0    <<  10) |           // [   10] Reserved - SBZ.
	    (0x1 << 9) | // [    9] ctrl_dfdqs. 0:Single-ended DQS,
			 // 1:Differential DQS
#if (DDR_GATE_LEVELING_EN == 1)
//        (0x1    <<   8) |           // [    8] ctrl_shgate. 0:Gate signal
//        length=burst length/2+N, 1:Gate signal length=burst length/2-1
#endif
#if defined(MEM_TYPE_LPDDR23)
//        (0x1    <<   8) |           // [    8] ctrl_shgate. 0:Gate signal
//        length=burst length/2+N, 1:Gate signal length=burst length/2-1
